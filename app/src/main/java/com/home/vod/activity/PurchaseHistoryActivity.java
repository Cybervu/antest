...
public class PurchaseHistoryActivity extends AppCompatActivity implements APICallManager.ApiInterafce {
    ...
    protected void onCreate(Bundle savedInstanceState) {
     ...
     /*   for(int i = 0 ;i<20 ;i++)
        {
            PurchaseHistory_Model purchaseHistoryModel = new PurchaseHistory_Model
    ...
    public void GetPurchaseHistoryDetails() {
        ...
        final APICallManager apiCallManager = new APICallManager(this, APIUrlConstant.PURCHASE_HISTORY_URL, parameters, APIUrlConstant.PURCHASE_HISTORY_URL_REQUEST_ID, APIUrlConstant.BASE_URl);
...
*/
    ...
    public void onTaskPostExecute(Object object, int requestID, String response) {
        if (APIUrlConstant.PURCHASE_HISTORY_URL_REQUEST_ID == requestID) {
            purchase_history(object, requestID, response);
        }
    }

    public void purchase_history(Object object, int requestID, String response) {

        PurchaseHistory_Model purchaseHistory_model = (PurchaseHistory_Model) object;
        ...
        Id_Purchase_History = new ArrayList<>();
        if (purchaseHistory_model.getCode() > 0) {
            if (purchaseHistory_model.getCode() == 200) {

                if (purchaseHistory_model != null && purchaseHistory_model.getSection().size() > 0) {

                    for (int i = 0; i < purchaseHistory_model.getSection().size(); i++) {
                        purchaseData.add(new PurchaseHistoryModel(purchaseHistory_model.getSection().get(i).getInvoiceId(), purchaseHistory_model.getSection().get(i).getId(), purchaseHistory_model.getSection().get(i).getTransactionDate(),
                                purchaseHistory_model.getSection().get(i).getTransactionStatus(), purchaseHistory_model.getSection().get(i).getAmount(), purchaseHistory_model.getSection().get(i).getStatusppv()));
                        Id_Purchase_History.add(purchaseHistory_model.getSection().get(i).getId());
                    }
                   /* for (PurchaseHistoryOutputModel model : purchaseHistoryOutputModel
                            ) {
