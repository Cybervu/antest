...
public class ShowWithEpisodesActivity extends AppCompatActivity implements GetTranslateLanguageAsync.GetTranslateLanguageInfoListner, LogoutAsynctask.Logout, GetLanguageListAsynTask.GetLanguageList,
   ...
   // ProgressBarHandler pDialog;
    ...
    public void onGetTranslateLanguagePreExecuteStarted() {
        Log.v("PINTU","translate pdlog show");
    ...
    public void onGetTranslateLanguagePostExecuteCompleted(String jsonResponse, int status) {
        try {
            if (pDialog != null && pDialog.isShowing()) {
                Log.v("PINTU","translate pdlog hide");
                pDialog.hide();
    ...
    public void onLogoutPostExecuteCompleted(int code, String status, String message) {
        try {
            if (pDialog != null && pDialog.isShowing()) {
                Log.v("PINTU","logoyut pdlog hide");
                pDialog.hide();
                ...
                if ((Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.IS_ONE_STEP_REGISTRATION, Util.DEFAULT_IS_ONE_STEP_REGISTRATION)
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            ...
                            Toast.makeText(ShowWithEpisodesActivity.this, Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.LOGOUT_SUCCESS, Util.DEFAULT_LOGOUT_SUCCESS), Toast.LENGTH_LONG).show();
                            finish();
    ...
    public void onGetContentDetailsPreExecuteStarted() {
        Log.v("PINTU","contentdetails pdlog show");

        pDialog.show();
        Log.v("SUBHA", "PREEXECUTE");
    }

    @Override
    public void onGetContentDetailsPostExecuteCompleted(ContentDetailsOutput contentDetailsOutput, int status, String message) {

        Log.v("SUBHA", "onGetContentDetailsPostExecuteCompleted");

        try {
            if (pDialog != null && pDialog.isShowing()) {
                Log.v("PINTU","contentdetails pdlog hide");
                pDialog.hide();
        ...
        if (status == 200) {
            ...
            Log.v("SUBHA", "permalink get contentdetails== === " + _permalink);
...
//        Log.v("SUBHA2","cast value" +castValue);
            ...
            if (movieReleaseDateStr.matches("") || movieReleaseDateStr.matches(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.NO_DATA, Util.DEFAULT_NO_DATA))) {
                ...
                videoReleaseDateTextView.setText(contentDetailsOutput.getReleaseDate());
            ...
            if (censorRatingStr.matches("") ||
                    censorRatingStr.matches(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.NO_DATA, Util.DEFAULT_NO_DATA))) {
            ...
            Log.v("SUBHA", "dd====== " + contentDetailsOutput.getSeason().length);
            ...
            if (contentDetailsOutput.getSeason() != null && contentDetailsOutput.getSeason().length > 0) {
                for (int j = 0; j < contentDetailsOutput.getSeason().length; j++) {
                    ...
                    season.add(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.SEASON, Util.DEFAULT_SEASON) + " " + contentDetailsOutput.getSeason()[j]);


                }
            }
            Log.v("SUBHA", "season====== " + season.size());
...
//                season_spinner.setBackgroundResource(R.drawable.spinner_theme);
            ...
            if (contentDetailsOutput.getBanner().trim().matches(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.NO_DATA, Util.DEFAULT_NO_DATA))) {

                if (contentDetailsOutput.getPoster().trim().matches(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.NO_DATA, Util.DEFAULT_NO_DATA))) {
                ...
                DisplayImageOptions options = new DisplayImageOptions.Builder().cacheInMemory(true)
                        ...
                        .showImageOnFail(R.drawable.logo)
                        .showImageOnLoading(R.drawable.logo).build();
                imageLoader.displayImage(contentDetailsOutput.getBanner().trim(), moviePoster, options);
    ...
    public void onGetEpisodeDetailsPreExecuteStarted() {
        Log.v("PINTU","getepisodedetails pdlog show");
    ...
    public void onGetEpisodeDetailsPostExecuteCompleted(Episode_Details_output episode_details_output, int status, int i, String message, String movieUniqueId) {
        ...
        try {
            if (pDialog != null && pDialog.isShowing()) {
                Log.v("PINTU","getepisodedetails pdlog hide");
                pDialog.hide();
        ...
        Log.v("SUBHA", "episode show...");

        String loggedInStr = preferenceManager.getUseridFromPref();
        if (status == 200) {
            Log.v("SUBHA", "episode show...1");

            Log.v("BISHAL", "itemdata==" + itemData);
            if (itemData.size() <= 0) {
    ...
    public void onGetEpisodeDetailsPostExecuteCompleted(ArrayList<Episode_Details_output> episode_details_output, int status, int i, String message,String movieUniqueId) {
        ...
        if (status == 200) {
            ...
            if (itemData.size() <= 0) {
                ...
                Log.v("SUBHA", "itemData show...1" + itemData.size());
                Log.v("SUBHA", "episode show... 2");
                if (pDialog != null && pDialog.isShowing()) {
                    pDialog.hide();
                   
                }

                Log.v("SUBHA", "data show...");
                seasontiveLayout.setVisibility(View.VISIBLE);
                seasontiveLayout.setLayoutManager(mLayoutManager);
                seasontiveLayout.setItemAnimator(new DefaultItemAnimator());
                EpisodesListAdapter mAdapter = new EpisodesListAdapter(ShowWithEpisodesActivity.this, R.layout.list_card_multipart, itemData, new EpisodesListAdapter.OnItemClickListener() {
...
*/
    ...
    public void onGetValidateUserPreExecuteStarted() {
        Log.v("PINTU","validateuser pdlog show");
    ...
    public void onGetValidateUserPostExecuteCompleted(ValidateUserOutput validateUserOutput, int status, String message) {
        ...
        try {
            if (pDialog != null && pDialog.isShowing()) {
                Log.v("PINTU","validate user pdlog hide");
                pDialog.hide();
    ...
    public void onAddToFavPostExecuteCompleted(AddToFavOutputModel addToFavOutputModel, int status, String sucessMsg) {
        ...
        if (pDialog.isShowing() && pDialog != null) {
            Log.v("PINTU","addd fav pdlog hide");
            pDialog.hide();
        }
    ...
    public void onDeleteFavPreExecuteStarted() {
        ...
        Log.v("PINTU","delete fav pdlog show");
    ...
    public void onDeleteFavPostExecuteCompleted(DeleteFavOutputModel deleteFavOutputModel, int status, String sucessMsg) {
        ...
        isFavorite = 0;
        if (pDialog.isShowing() && pDialog != null) {
            Log.v("PINTU","delete fav pdlog hide");
            pDialog.hide();
        }
    ...
    public void onViewContentRatingPreExecuteStarted() {
        Log.v("PINTU","view content rating pdlog show");
    ...
    public void onViewContentRatingPostExecuteCompleted(ViewContentRatingOutputModel viewContentRatingOutputModel,
        ...
        try {
            if (pDialog != null && pDialog.isShowing()) {
                Log.v("PINTU","view content pdlog hide");
                pDialog.hide();
        ...
        if (status == 200) {
            ...
            String loggedInStr = preferenceManager.getUseridFromPref();
            Log.v("BISHAL", "review data" + reviews);
            Log.v("BISHAL", "rating data" + rating);
            ...
            if (rating.equalsIgnoreCase("0")) {
                ...
                if (preferenceManager.getLoginFeatureFromPref() == 1) {
                    ...
                    if (loggedInStr == null) {
                        ...
                        Log.v("BISHAL", "rating 0 ==== " + viewContentRatingOutputModel.getShowrating());
                    } else {
                        if (viewContentRatingOutputModel.getShowrating() == 1) {
                            viewRatingTextView.setText(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.ADD_A_REVIEW, Util.DEFAULT_ADD_A_REVIEW));
                            Log.v("BISHAL", "rating 1 ==== " + viewContentRatingOutputModel.getShowrating());
                        } else {
                            Log.v("BISHAL", "rating 2 ==== " + viewContentRatingOutputModel.getShowrating());
    ...
    public void onVideoDetailsPreExecuteStarted() {
        Log.v("PINTU","videodetails pdlog show");
    ...
    public void onVideoDetailsPostExecuteCompleted(Get_Video_Details_Output get_video_details_output, int statusCode, String stus, String message) {
     ...
     /*check if status code 200 then set the video url before this it check it is thirdparty url or normal if third party
        ...
        try {
            if (pDialog != null && pDialog.isShowing()) {
                Log.v("PINTU","videodetails pdlog hide");
                pDialog.hide();
               
            }
        } catch (IllegalArgumentException ex) {
            playerModel.setVideoUrl(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.NO_DATA, Util.DEFAULT_NO_DATA));
        }
            ...
            if (get_video_details_output.getThirdparty_url() == null || get_video_details_output.getThirdparty_url().matches("")) {
                ...
                if (get_video_details_output.getStudio_approved_url() != null &&
                    ...
                    if (get_video_details_output.getVideoUrl().trim() != null && !get_video_details_output.getVideoUrl().isEmpty() && !get_video_details_output.getVideoUrl().equals("null") && !get_video_details_output.getVideoUrl().trim().matches("")) {
                        ...
                        playerModel.setMpdVideoUrl(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.NO_DATA, Util.DEFAULT_NO_DATA));
                    }
                } else {
...
//        SharedPreferences isLoginPref = getSharedPreferences(Util.IS_LOGIN_SHARED_PRE, 0); // 0 - for private mode
        ...
        season_spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                ...
                Log.v("SUBHA", "episode details  call 1");
                ...
                episodeDetailsInput.setLang_code(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.SELECTED_LANGUAGE_CODE, com.home.vod.util.Util.DEFAULT_SELECTED_LANGUAGE_CODE));
...
*/
        ...
        viewRatingTextView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                reviewIntent.putExtra("muviId", movieUniqueId);
        ...
        Log.v("SUBHA", "PER" + getIntent().getStringExtra(Util.PERMALINK_INTENT_KEY));
                            ...
                            if (loggedinDateStr != null) {
                                ...
                                if (differenceInDays >= 7) {
                                    ...
                                    if (Util.checkNetwork(ShowWithEpisodesActivity.this) == true) {
                                        ...
                                        validateUserInput.setLanguageCode(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.SELECTED_LANGUAGE_CODE, Util.DEFAULT_SELECTED_LANGUAGE_CODE));
        ...
        btnmore.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                Log.v("BKS","view more permalink=="+_permalink);
            ...
            case R.id.action_logout:
                ...
                dlgAlert.setPositiveButton(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.YES, Util.DEFAULT_YES), new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        logoutInput.setLang_code(Util.getTextofLanguage(ShowWithEpisodesActivity.this, Util.SELECTED_LANGUAGE_CODE, Util.DEFAULT_SELECTED_LANGUAGE_CODE));
